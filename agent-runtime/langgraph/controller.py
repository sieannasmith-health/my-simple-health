...
def _event(state: AgentOSState, event_type: str, **payload):
    return {"event_type": event_type, "objective_id": state["objective_id"], "correlation_id": state["correlation_id"], "agent": state["current_agent"], "stage": state["current_stage"], "evidence_version": state["evidence_version"], **payload}


def classify_failure(state: AgentOSState):
    failure_class = classify_reason(state.get("reason_code", ""))
    return {"failure_class": failure_class, "recovery_owner": recovery_owner_for(failure_class, {"assigned_agent": state.get("current_agent")}), "failed_stage": state.get("failed_stage") or state["current_stage"], "failed_agent": state.get("failed_agent") or state["current_agent"], "status": "recovery_required"}


def recover(state: AgentOSState):
    return {"attempt": 0, "redrive_count": state["redrive_count"] + 1, "current_agent": state.get("recovery_owner", state["current_agent"]), "status": "recovering", "resume_from": state.get("failed_stage", state["current_stage"]), "reason_code": ""}
...
def refinement_exhausted(state: AgentOSState):
    return {"status": "blocked", "reason_code": "MAX_REDRIVE_EXCEEDED", "failure_class": "terminal", "recovery_owner": "nomy", "failed_stage": "QA", "failed_agent": "tessa"}


def accept(state: AgentOSState):
    return {"status": "completed", "current_agent": "nomy", "current_stage": "ACCEPTANCE", "reason_code": ""}


def human_gate(state: AgentOSState):
    request = state.get("human_gate") or {"reason_code": state.get("reason_code", "INPUT_REQUIRED"), "failed_stage": state.get("failed_stage", state["current_stage"]), "failed_agent": state.get("failed_agent", state["current_agent"])}
    response = interrupt(request)
    return {"status": "recovery_required", "human_gate": {}, "reason_code": "", "qa_feedback": f"Human gate resumed: {response}"}
...
def route_failure(state: AgentOSState):
    ...
    if kind == "quality": return "refine" if state["redrive_count"] < state["max_redrives"] else "refinement_exhausted"
    if kind in {"transient", "dependency"}: return "recover" if state["redrive_count"] < state["max_redrives"] else END
    if kind == "authorization": return "recover" if state["redrive_count"] < state["max_redrives"] else "human_gate"
    if kind == "human_input": return "human_gate"
    return END


def route_qa(state: AgentOSState):
    if state["qa_status"] == "pass": return "accept"
    if state["qa_status"] == "fail":
        failure_class = classify_reason(state.get("reason_code", "QA_FAILED"))
        if failure_class != "quality": return "classify_failure"
...
def build_graph(checkpointer=None, ports: AgentOSPorts | None = None):
    ...
    def tessa_evaluate(state: AgentOSState):
        ...
        qa_status = result.get("qa_status", "fail"); feedback = str(result.get("qa_feedback") or "")
        active_ports.emit_event(_event(qa_state, "qa.finished", qa_status=qa_status))
        updates = {"current_agent": "tessa", "current_stage": "QA", "qa_status": qa_status if qa_status in {"pass", "fail", "not_run"} else "fail", "qa_feedback": feedback}
