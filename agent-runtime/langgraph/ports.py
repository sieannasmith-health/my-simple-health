...
def fail_closed_ports() -> AgentOSPorts:
    """Safe default when execution adapters have not been configured."""

    def execute(_state: AgentOSState) -> dict[str, Any]:
        return {
            "status": "blocked",
            "reason_code": "EXECUTION_ADAPTER_NOT_CONFIGURED",
            "message": "LangGraph execution adapter is not configured.",
        }

    def qa(_state: AgentOSState) -> dict[str, Any]:
        return {
            "qa_status": "fail",
            "qa_feedback": "LangGraph QA adapter is not configured.",
            "reason_code": "QA_ADAPTER_NOT_CONFIGURED",
        }
...
class ScriptedPorts:
    """Deterministic no-write adapter for shadow and conformance runs."""

    def __init__(
        self,
        execution_results: Mapping[str, Sequence[Mapping[str, Any]]] | None = None,
        qa_results: Sequence[Mapping[str, Any]] | None = None,
    ) -> None:
    ...
    def execute_agent(self, state: AgentOSState) -> dict[str, Any]:
        ...
        if not queue:
            return {
                ...
                "message": f"No scripted execution evidence for {state.get('current_agent')}",
            }
    ...
    def evaluate_qa(self, _state: AgentOSState) -> dict[str, Any]:
        if not self.qa_results:
            return {
                "qa_status": "fail",
                "qa_feedback": "No scripted Tessa evaluation evidence.",
                "reason_code": "MISSING_EVIDENCE",
            }
...
class JsonSubprocessExecutionPort:
    ...
    shell=False is intentional. Credentials and tool authority stay outside
    LangGraph state and are supplied only by the caller environment.
    """
    ...
    def __call__(self, state: AgentOSState) -> dict[str, Any]:
        ...
        if completed.returncode != 0:
            return {
                ...
                "message": completed.stderr.strip() or f"Execution adapter exited {completed.returncode}",
            }
        ...
        try:
            ...
            return {
                "status": "blocked",
                "reason_code": "INVALID_EXECUTION_EVIDENCE",
                "message": "Execution adapter returned invalid JSON.",
            }
        if not isinstance(result, dict):
            return {
                "status": "blocked",
                "reason_code": "INVALID_EXECUTION_EVIDENCE",
                "message": "Execution adapter response must be a JSON object.",
            }
        return result
