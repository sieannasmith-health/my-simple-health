...
export class DurableKernel {
  ...
  public async claimLease(workerId: string, role: string, idempotencyKey: string): Promise<WorkerClaimResult | null> {
    return this.transaction(async (client) => {
      const duplicate = await client.query('SELECT event_id FROM event_ledger WHERE idempotency_key = $1', [idempotencyKey]);
      if ((duplicate.rowCount ?? 0) > 0) return null;

      const job = await client.query(
        `SELECT t.task_id, t.version, p.lease_timeout_seconds, p.max_attempts
           FROM tasks t
           JOIN retry_policies p ON p.policy_id = t.retry_policy_id
          WHERE t.assigned_worker_role = $1
            AND t.status = 'READY'
      ...
      if (attemptNumber > Number(row.max_attempts)) {
        const failed = await client.query(
          `UPDATE tasks SET status = 'TERMINAL_FAILURE', version = version + 1, updated_at = CURRENT_TIMESTAMP
            WHERE task_id = $1 AND status = 'READY' AND version = $2`,
      ...
      const updated = await client.query(
        `UPDATE tasks SET status = 'LEASED', version = version + 1, next_eligible_at = NULL, updated_at = CURRENT_TIMESTAMP
          WHERE task_id = $1 AND status = 'READY' AND version = $2`,
        [row.task_id, row.version],
      );
      ...
      await client.query(
        `INSERT INTO task_attempts
          (task_id, worker_id, attempt_number, status, idempotency_key, lease_token_hash, lease_expires_at)
         VALUES ($1, $2, $3, 'ACTIVE', $4, $5, $6)`,
        [row.task_id, workerId, attemptNumber, idempotencyKey, this.hashToken(leaseToken), leaseExpiresAt],
      );
  ...
  public async startAttempt(taskId: string, leaseToken: string, idempotencyKey: string): Promise<void> {
    await this.transaction(async (client) => {
      ...
      const updated = await client.query(
        `UPDATE tasks SET status = 'RUNNING', version = version + 1, updated_at = CURRENT_TIMESTAMP
          WHERE task_id = $1 AND status = 'LEASED' AND version = $2`,
        [taskId, task.rows[0].version],
      );
    ...
    return this.transaction(async (client) => {
      if (await this.eventExists(client, idempotencyKey)) {
        const existing = await client.query(
          `SELECT lease_expires_at FROM task_attempts WHERE task_id = $1 AND lease_token_hash = $2`,
          [taskId, this.hashToken(leaseToken)],
        );
      ...
      const policy = await client.query(
        `SELECT p.lease_timeout_seconds FROM tasks t JOIN retry_policies p ON p.policy_id = t.retry_policy_id WHERE t.task_id = $1`,
        [taskId],
      );
      const leaseExpiresAt = new Date(Date.now() + Number(policy.rows[0].lease_timeout_seconds) * 1000);
      await client.query(
        `UPDATE task_attempts SET last_heartbeat_at = CURRENT_TIMESTAMP, lease_expires_at = $1 WHERE attempt_id = $2`,
        [leaseExpiresAt, attempt.attempt_id],
      );
  ...
  public async submitCompletion(taskId: string, leaseToken: string, artifacts: ArtifactPayload[], idempotencyKey: string): Promise<void> {
    await this.transaction(async (client) => {
      ...
      await client.query("UPDATE task_attempts SET status = 'COMPLETED', finished_at = CURRENT_TIMESTAMP WHERE attempt_id = $1 AND status = 'ACTIVE'", [attempt.attempt_id]);
      const updated = await client.query(
        `UPDATE tasks SET status = 'VALIDATING', version = version + 1, updated_at = CURRENT_TIMESTAMP
          WHERE task_id = $1 AND status = $2 AND version = $3`,
        [taskId, task.rows[0].status, task.rows[0].version],
      );
  ...
  public async executeReclamationSweep(): Promise<void> {
    await this.transaction(async (client) => {
      const expired = await client.query(
        `SELECT a.attempt_id, a.task_id, t.status, t.version
           FROM task_attempts a JOIN tasks t ON t.task_id = a.task_id
          WHERE a.status = 'ACTIVE' AND a.lease_expires_at < CURRENT_TIMESTAMP
          FOR UPDATE OF a, t SKIP LOCKED`,
      );
      for (const attempt of expired.rows) {
        if (!['LEASED', 'RUNNING'].includes(attempt.status)) continue;
        await client.query(
          `UPDATE task_attempts SET status = 'EXPIRED', finished_at = CURRENT_TIMESTAMP, failure_class = 'TIMEOUT'
            WHERE attempt_id = $1 AND status = 'ACTIVE'`,
          [attempt.attempt_id],
        );
        const updated = await client.query(
          `UPDATE tasks SET status = 'RETRYABLE_FAILURE', version = version + 1, updated_at = CURRENT_TIMESTAMP
            WHERE task_id = $1 AND status = $2 AND version = $3`,
          [attempt.task_id, attempt.status, attempt.version],
        );
      ...
      const retryable = await client.query(
        `SELECT t.task_id, t.version, p.max_attempts, p.initial_backoff_seconds, p.max_backoff_seconds, p.backoff_multiplier
           FROM tasks t JOIN retry_policies p ON p.policy_id = t.retry_policy_id
          WHERE t.status = 'RETRYABLE_FAILURE'
          FOR UPDATE OF t SKIP LOCKED`,
      );
      for (const task of retryable.rows) {
        ...
        if (attempts >= Number(task.max_attempts)) {
          const updated = await client.query(
            `UPDATE tasks SET status = 'TERMINAL_FAILURE', version = version + 1, updated_at = CURRENT_TIMESTAMP
              WHERE task_id = $1 AND status = 'RETRYABLE_FAILURE' AND version = $2`,
        ...
        const backoff = Math.min(
          Number(task.max_backoff_seconds),
          Math.round(Number(task.initial_backoff_seconds) * Math.pow(Number(task.backoff_multiplier), exponent)),
        );
        const updated = await client.query(
          `UPDATE tasks
              SET status = 'READY', version = version + 1,
                  next_eligible_at = CURRENT_TIMESTAMP + ($3 * INTERVAL '1 second'), updated_at = CURRENT_TIMESTAMP
            WHERE task_id = $1 AND status = 'RETRYABLE_FAILURE' AND version = $2`,
          [task.task_id, task.version, backoff],
        );
  ...
  private async lockActiveAttempt(client: PoolClient, taskId: string, leaseToken: string) {
    const result = await client.query(
      `SELECT attempt_id FROM task_attempts
        WHERE task_id = $1 AND lease_token_hash = $2 AND status = 'ACTIVE' AND lease_expires_at > CURRENT_TIMESTAMP
        FOR UPDATE`,
      [taskId, this.hashToken(leaseToken)],
    );
  ...
  private async eventExists(client: PoolClient, idempotencyKey: string): Promise<boolean> {
    const result = await client.query('SELECT 1 FROM event_ledger WHERE idempotency_key = $1', [idempotencyKey]);
    return (result.rowCount ?? 0) > 0;
  }

  private async appendEvent(client: PoolClient, idempotencyKey: string, eventType: string, taskId: string, payload: unknown): Promise<void> {
    await client.query(
      `INSERT INTO event_ledger (idempotency_key, event_type, associated_task_id, payload) VALUES ($1, $2, $3, $4::jsonb)`,
      [idempotencyKey, eventType, taskId, JSON.stringify(payload ?? {})],
    );
  }
}
