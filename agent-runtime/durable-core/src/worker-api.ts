...
export function createWorkerApi(kernel: DurableKernel, identity?: IdentityAuthorityService) {
  ...
  if (identity) {
    app.use(asyncRoute(async (req: AuthenticatedRequest, _res, next) => {
      ...
      req.worker = await identity.authenticateBearer(secret);
      next();
    }));
  }
  ...
  app.post('/v1/tasks/:taskId/start', asyncRoute(async (req: AuthenticatedRequest, res) => {
    const params = z.object({ taskId }).parse(req.params); const body = leasedTaskSchema.parse(req.body);
    await authorize(identity, requireWorker(req, identity), params.taskId, 'WRITE');
  ...
  app.use((error: unknown, _req: Request, res: Response, _next: NextFunction) => {
    ...
    if (/Authority denied|Capability denied/.test(message)) return res.status(403).json({ error: 'FORBIDDEN' });
...
async function authorize(identity: IdentityAuthorityService | undefined, worker: AuthenticatedWorker, taskIdValue: string, authority: WorkerAuthority): Promise<void> {
  if (identity) await identity.authorizeTaskMutation(worker, taskIdValue, authority);
}

function asyncRoute(handler: (req: any, res: Response, next: NextFunction) => Promise<Response | void>) {
