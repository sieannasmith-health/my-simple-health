...
export const MSH_AGENT_REGISTRY: readonly MshAgentMetadataCard[] = [
  {
    id: 'nomy',
    name: 'Nomy',
    role: 'Product Orchestration',
    owner: 'MSH',
    version: '1.0.0',
    status: 'active',
    capabilities: ['product_orchestration', 'objective_decomposition', 'product_acceptance'],
    explicitNonCapabilities: ['implementation_merge', 'repository_admin', 'secrets_admin', 'destructive_repository_actions'],
  ...
  },
  {
    ...
    version: '1.0.0',
    status: 'active',
    capabilities: ['software_engineering', 'repository_implementation', 'ci_integration'],
    explicitNonCapabilities: ['merge', 'repository_admin', 'secrets_admin', 'destructive_repository_actions'],
    protocolBindings: ['TEMPORAL_INTERNAL', 'MCP_TOOL_ACCESS'],
    allowedTools: SELAH_TOOLS,
    a2aExposure: 'none',
  },
  {
    id: 'tessa',
    ...
    version: '1.0.0',
    status: 'active',
    capabilities: ['quality_engineering', 'test_review', 'acceptance_verification'],
  ...
  registered('reese', 'Reese', 'Regulatory & Compliance', ['regulatory_review', 'compliance']),
...
if (byId.size !== MSH_AGENT_REGISTRY.length) {
  throw new Error('DUPLICATE_AGENT_ID');
}

export function getAgentCard(agentId: string): MshAgentMetadataCard | undefined {
...
export function canAgentUseTool(agentId: string, tool: McpGitHubTool): boolean {
  const card = getAgentCard(agentId);
  return card?.status === 'active' && card.allowedTools.includes(tool) === true;
}

export function assertAgentToolPermission(agentId: string, tool: McpGitHubTool): void {
  if (!canAgentUseTool(agentId, tool)) {
    throw new Error(`AGENT_TOOL_FORBIDDEN:${agentId}:${tool}`);
  }
}
