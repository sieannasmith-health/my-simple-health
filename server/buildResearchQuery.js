...
`,
        ...
        if (!response.ok) {
            console.error("Research query builder API error:", data);
            return cleanQuestion;
        }

        const researchQuery = extractOutputText(data);
        if (!researchQuery) return cleanQuestion;

        return researchQuery.replace(/```/g, "").replace(/\s+/g, " ").trim();
    } catch (error) {
