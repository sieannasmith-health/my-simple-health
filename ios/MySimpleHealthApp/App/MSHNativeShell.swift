...
struct MSHBottomTabBar: View {
    ...
    var body: some View {
        ...
        .overlay(alignment: .top) {
            LinearGradient(
                colors: [
                    Color(red: 0.78, green: 0.46, blue: 1).opacity(0.46),
                    Color.white.opacity(0.58),
                    Color(red: 0.42, green: 0.82, blue: 1).opacity(0.46)
                ],
                startPoint: .leading,
                endPoint: .trailing
            )
        ...
        .shadow(color: Color(red: 0.44, green: 0.78, blue: 1).opacity(0.11), radius: 12, y: -2)
...
private struct MSHBottomTabButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .shadow(
                color: Color(red: 0.48, green: 0.82, blue: 1)
        ...
        NavigationStack {
            ZStack {
                ...
                Group {
                    switch section {
                    case .myHealth:
                        if let notificationRoute { MSHNativeNotificationRouteScreen(route: notificationRoute) }
                        else { MSHMyHealthHomeScreen() }
                    case .explore:
                        if let notificationRoute { MSHNativeNotificationRouteScreen(route: notificationRoute) }
                        else { MSHExploreScreen() }
                    case .simple: MSHSimpleScreen()
                    case .progress:
                        if let notificationRoute { MSHNativeNotificationRouteScreen(route: notificationRoute) }
                        else { MSHProgressScreen() }
...
private struct MSHNativeNotificationRouteScreen: View {
    ...
    @ViewBuilder
    var body: some View {
        if let destination = nativeDestination {
            MSHNativeFeatureScreen(destination: destination)
        } else {
            switch route.appSection {
            case .myHealth: MSHMyHealthHomeScreen()
            case .explore: MSHExploreScreen()
            case .simple: MSHSimpleScreen()
            case .progress: MSHProgressScreen()
            case .me: MSHProfileSettingsScreen()
            }
        }
    }
                ...
                VStack(alignment: .leading, spacing: 28) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack(alignment: .bottom, spacing: 10) {
                            ...
                            Button { MSHNativeHaptic.selection.play() } label: {
...
private struct MSHSimplePromptRow: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            Image(systemName: image).foregroundStyle(MSHColor.accent).frame(width: 28)
            Text(title)
...
struct MSHWebFeatureScreen: View {
    ...
    var body: some View { MSHNativeFeatureScreen(destination: destination) }
}
        ...
        ZStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 28) {
                    ...
                    .overlay { RoundedRectangle(cornerRadius: 24).stroke(MSHColor.border.opacity(0.72), lineWidth: 0.8) }
                }
                .padding(.horizontal, 20)
                .padding(.top, 18)
                .padding(.bottom, 36)
...
private extension MSHFeatureDestination {
    ...
    var nativeStatusCopy: String {
        switch self {
        case .calendar: "Calendar is native and persists appointments, movement, medication actions, cycle context, and personal events on this iPhone."
        case .movementPlan, .movementLibrary: "Movement now has a native destination boundary. Existing saved movement data can be connected here without an embedded webpage."
        case .cycle: "Cycle stays native so cycle context can be rebuilt as an iPhone experience."
        case .medications: "Medication continuity has a native destination boundary for the existing medication capability."
        case .landscape, .selfInsight, .horizon, .path, .practice, .discovery, .journey, .healthStory:
            "The existing Journey and self-understanding capability is preserved underneath this native destination while its presentation is reconstructed for iOS."
        case .food: "This is the native home for Food: household inventory, nutrition context, waste reduction, meal planning, grocery-list creation, and future shopping handoff."
        case .financialHealth: "The existing Financial Core remains protected underneath this native destination while the contextual financial-health experience is built around it."
        case .explore: "Explore is already native and remains the place to browse MSH capabilities beyond what My Health selectively serves up."
        case .myHealth: "Apple Health remains native and connected to My Health."
        }
    }
}
...
private struct MSHAppleHealthConnectionScreen: View {
        ...
        ZStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 26) {
                    VStack(alignment: .leading, spacing: 10) {
                        Text("APPLE HEALTH")
                            .font(.caption2.weight(.semibold)).tracking(2).foregroundStyle(MSHColor.sage)
                        Text("Your connected health data.")
                            .font(.system(size: 34, weight: .regular, design: .serif)).foregroundStyle(MSHColor.charcoal)
                        Text("Apple Health brings movement, sleep, heart activity, and body measurements into My Simple Health as context.")
                            .font(.body).foregroundStyle(MSHColor.charcoal.opacity(0.66))
                    }
                    ...
                    if let status {
                        VStack(alignment: .leading, spacing: 14) {
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text(status.isConnected ? "\(status.selectedAreas.count) health areas selected" : "Connect Apple Health when you’re ready.")
                                        .font(.subheadline).foregroundStyle(MSHColor.charcoal.opacity(0.62))
                                }
                                Spacer()
                                Image(systemName: status.isConnected ? "checkmark.circle.fill" : "heart.text.square")
                                    .font(.title2).foregroundStyle(MSHColor.sage)
                            }
                            if let lastSync = status.lastSuccessfulSyncAt {
                                ...
                                .font(.caption).foregroundStyle(MSHColor.charcoal.opacity(0.58))
                            }
                        }
                        .padding(18)
                        .background(Color.white.opacity(0.54))
                        .clipShape(RoundedRectangle(cornerRadius: 22, style: .continuous))
                        .overlay { RoundedRectangle(cornerRadius: 22).stroke(Color.black.opacity(0.06), lineWidth: 0.8) }

                        VStack(alignment: .leading, spacing: 0) {
                            Text("CONNECTED AREAS")
                                .font(.caption2.weight(.semibold)).tracking(1.5).foregroundStyle(MSHColor.charcoal.opacity(0.54))
                                .padding(.bottom, 10)
                            ForEach(MSHHealthArea.allCases) { area in
                                HStack(spacing: 14) {
                                    Image(systemName: area.systemImage).foregroundStyle(MSHColor.sage).frame(width: 28)
                                    Text(area.title).font(.system(.body, design: .serif)).foregroundStyle(MSHColor.charcoal)
                                    Spacer()
                                ...
                                .padding(.vertical, 13)
                                if area != MSHHealthArea.allCases.last { Divider() }
                            }
                        }

                        Button { Task { await performPrimaryAction(connected: status.isConnected) } } label: {
                            HStack(spacing: 9) {
                                if isWorking { ProgressView().tint(.white) }
                                Text(status.isConnected ? "Refresh Apple Health" : "Connect Apple Health").font(.headline)
                            }
                            .foregroundStyle(.white).frame(maxWidth: .infinity).frame(height: 52)
                            .background(MSHColor.charcoal).clipShape(RoundedRectangle(cornerRadius: 17, style: .continuous))
                        }
                        .buttonStyle(.plain)
                        .disabled(isWorking)
                    } else {
                        ProgressView("Loading Apple Health…").tint(MSHColor.sage).frame(maxWidth: .infinity).padding(.vertical, 50)
                    }
                    ...
                    Text("Your Apple Health permissions remain controlled by iOS. My Simple Health only reads the areas you have allowed.")
                        .font(.footnote).foregroundStyle(MSHColor.charcoal.opacity(0.54))
                }
    ...
    private func loadStatus() async {
        do {
            status = MSHAppleHealthStatus(syncState: try await dataSource.loadStatus())
            errorMessage = nil
        } catch {
    ...
    private func performPrimaryAction(connected: Bool) async {
        ...
        do {
            if connected { try await MSHAppleHealthRuntime.refreshConnectedHealth() }
            else { _ = try await MSHAppleHealthRuntime.connectForOnboarding() }
            await loadStatus()
        } catch {
                ...
                VStack(alignment: .leading, spacing: 28) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        TextField("Name or nickname", text: $displayName)
                            ...
                            .background(MSHColor.controlFill).clipShape(RoundedRectangle(cornerRadius: 16))
                    }
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Picker("My Space", selection: $mySpaceRawValue) {
                            ForEach(MSHMySpace.allCases) { space in Text(space.title).tag(space.rawValue) }
                        }
