...
public actor HealthSyncCoordinator {
    ...
    public func sync(areas requestedAreas: Set<HealthDataArea>? = nil) async throws -> HealthSyncBatch {
        ...
        do {
            ...
            mshCoordinatorDiagnostic("provider_sync_complete", "recordCount=\(batch.records.count) deletedCount=\(batch.deletedSourceRecordIDs.count) checkpointCount=\(batch.checkpoints.count) failureCount=\(batch.partialFailures.count)")
