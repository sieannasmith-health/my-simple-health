...
(function () {
  ...
  function renderTools() {
    renderGlass({
      ...
      body:`<div class="msh-tools-directory msh-glide" data-msh-interaction="glide" data-msh-glide-label="Tools" data-msh-glide-item="tool">
        ...
        <section aria-labelledby="activity-tools"><p class="msh-glass-category">Connected health</p><h2 id="activity-tools">Activity</h2><p>See movement recorded by Apple Health today, across workouts, and over recent time—without goals, streaks, or compliance scoring.</p><a href="my-health.html?view=activity">Open Activity <span aria-hidden="true">→</span></a></section>
    ...
    if (firstDoorStep === 'sleep' && intent) {
      renderGlass({
        ...
        choices:[
          ...
          { id:'sleep:other', label:'Something else', detail:'Describe it in your own way.' }
        ],
  ...
  function prepareProjectFocus(entry) {
    if (!entry || entry.intent !== 'work_on_something' || !entry.context) return null;
    return MSHStorage.saveFocusDecision({
      label:entry.context,
      navigationState:'develop',
      subjectType:'user_named_subject',
      subjectId:`first-door:${entry.id}`,
      sourceType:'first_door',
      sourceId:entry.id
    });
  }
  ...
  function healthMapLayers(state) {
    ...
    const currentFocuses = MSHStorage.getCurrentFocuses
      ? MSHStorage.getCurrentFocuses(state)
      : state.focuses.filter(item => item.status === 'active');
    const focus = currentFocuses[0] || null;
    const dispositionLabels = {
      develop:'I want to work on this',
      preserve:'I want to protect what is working',
      explore:'I want to understand this better',
      prepare:'I want to keep an eye on this',
      adapt:'I need to adapt around this',
      no_action:'I want to leave this alone for now'
    };
    return [
      {
  ...
  root.addEventListener('click', event => {
    ...
    if (route) {
      ...
      const prepared = prepareProjectFocus(entry);
      if (prepared && prepared.navigationState === 'develop') {
        const competing = MSHStorage.getCurrentFocuses().filter(item => item.navigationState === 'develop');
        if (competing.length > 1) {
          event.preventDefault();
          location.href = `my-landscape.html?capacityFocus=${encodeURIComponent(prepared.id)}`;
        }
      }
    }
  });
