...
(function () {
  ...
  function focus(st) { return st.focuses.find(x => x.status === 'active') || null; }
  ...
  function render() {
    const st = state(); const project = active(); const past = history(st);
    mount.innerHTML = `<section class="msh-project-header"><p class="msh-eyebrow">Path · Point A → Point B</p><h1>${project ? 'Your path is visible.' : 'Choose one path worth walking.'}</h1><p>A Project turns a direction into a meaningful season of movement. It can pause, change, or complete without disappearing.</p>${notice ? `<div class="msh-lifecycle-notice" role="status">${esc(notice)}</div>` : ''}</section>
      ${project ? renderActive(project, st) : renderCreate(focus(st), vision(st), past)}
      ${renderHistory(past)}`;
    activatePathCanvas();
  }
  ...
  mount.addEventListener('submit', event => {
    ...
    event.preventDefault(); const fd = new FormData(event.target); const createdAt = new Date().toISOString(); const id = storage.uid('project');
    storage.updateState(st => {
      st.projects.forEach(x => { if (x.status === 'active') { x.status = 'paused'; x.updatedAt = createdAt; } });
      st.projects.push({ id, status:'active', title:fd.get('title').trim(), pointA:fd.get('pointA').trim(), pointB:fd.get('pointB').trim(), why:fd.get('why').trim(), capacity:fd.get('capacity') || '', milestone:fd.get('milestone').trim(), createdAt, updatedAt:createdAt });
