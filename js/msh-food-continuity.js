...
(function (global) {
  ...
  function ensureFoodForGrocery(food, grocery, timestamp) {
    ...
    if (item) {
      if (item.status === 'archived') { item.status = 'active'; item.restoredAt = timestamp; }
      if (!item.productId && grocery.productId) item.productId = grocery.productId;
      return item;
    }
    item = { id: uid('food'), productId: grocery.productId || null, name: text(grocery.name) || 'Unnamed food', category: grocery.category || 'Other', status: 'active', source: 'grocery_purchase', createdAt: timestamp, updatedAt: timestamp };
  ...
  function suggestRestock(state, foodId, options) {
    ...
    const grocery = { id: uid('grocery'), productId: directoryFood.productId || null, name: directoryFood.name, quantity: settings.quantity || null, reason: settings.reason || 'Restock after using what was on hand', estimatedPrice: settings.estimatedPrice == null ? null : Number(settings.estimatedPrice), status: 'active', source: 'inventory_continuity', createdAt: timestamp };
  ...
  const API = Object.freeze({ ensureFoodState, purchaseGroceryItem, suggestRestock, markStockUsedUp });
