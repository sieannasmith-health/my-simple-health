...
(function (global) {
  ...
  const EVENT_META = Object.freeze({
    event:{label:'Event',layer:'events',icon:'calendar'},
    cycle:{label:'Cycle',layer:'cycle',icon:'cycle'}, movement:{label:'Movement',layer:'movement',icon:'movement'}, symptom:{label:'Symptoms',layer:'symptoms',icon:'symptom'}, care:{label:'Care & appointments',layer:'care',icon:'care'},
    medication:{label:'Medication',layer:'medications',icon:'medication'}, sexualHealth:{label:'Sexual health',layer:'sexualHealth',icon:'sexualHealth'}, measurement:{label:'Measurement',layer:'measurements',icon:'measurement'},
    practice:{label:'Practice',layer:'practices'}, project:{label:'Project',layer:'projects'}, prevention:{label:'Preventive care',layer:'care'},
    note:{label:'Observation',layer:'observations',icon:'observation'}
  });
  ...
  function eventCategory(event) {
    ...
    if (/^event$|birthday|celebration|holiday|travel|school|work|vacation/.test(value)) return 'event';
  ...
  function deriveHealthEvents(state, importedEvents=[], range=null) {
    ...
    const add=(event,sourceKind) => {
      ...
      processedCount+=1;
      represented.add(id);
      const item={id,date,category,sourceKind,title:event.title||event.statement||event.label||meta.label,detail:event.detail||event.note||'',photo:event.photo||null,recordStatus:event.recordStatus||'recorded',informationClass:event.informationClass||'RECORDED',startAt:event.startAt||event.timestamp||'',movement:event.movement||null};
