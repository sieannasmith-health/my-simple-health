...
(function () {
  ...
  const navItems = [
    ...
    { key: 'vision', label: 'Vision', href: 'my-vision.html' },
  ...
  function renderNav(activePage, mobile) {
    return navItems.map(item => {
      if (item.comingSoon) return `<span class="is-coming-soon" aria-disabled="true" title="Coming next">${item.label}</span>`;
  ...
  function mountShell() {
    ...
    if (headerMount) {
      headerMount.innerHTML = `<header class="msh-app-header"><div class="msh-app-header-inner"><a class="msh-app-logo" href="index.html">My Simple Health</a><nav class="msh-app-nav" aria-label="My Health workspace">${renderNav(activePage,false)}</nav><a class="msh-hello-launcher" href="hello.html">Hello <span aria-hidden="true">→</span></a></div></header>`;
    }
    if (mobileMount) {
      mobileMount.innerHTML = `<nav class="msh-mobile-nav" aria-label="My Health mobile navigation">${renderNav(activePage,true)}<a href="hello.html">Hello</a></nav>`;
    }
  }
