...
test('legacy and supporting public surfaces retain style.css during isolation', async () => {
  ...
  const assessments = await readFile(new URL('../assessments.html', import.meta.url), 'utf8');
  const wheel = await readFile(new URL('../wellness-wheel.html', import.meta.url), 'utf8');
  assert.match(home, /href=["']css\/msh-public-story\.css["']/);
  assert.match(assessments, /href=["']css\/msh-foundation\.css["']/);
