...
test('routes each need to an existing capability instead of enforcing framework order', () => {
  const firstDoor = loadFirstDoor();
  assert.match(firstDoor.getIntent('health_question').primary.href, /hello\.html/);
  assert.match(firstDoor.getIntent('not_working').primary.href, /hello\.html/);
  assert.match(firstDoor.getIntent('care_support').primary.href, /hello\.html/);
...
test('care routing states the current prototype limitation honestly', () => {
  const firstDoor = loadFirstDoor();
  assert.match(firstDoor.getIntent('care_support').orientation, /does not yet provide a verified provider or services directory/i);
});
